...
class _RestaurantDetailPageState extends State<RestaurantDetailPage> {
  ...
  Future<void> fetchRestaurantDetails() async {
    try {
      final res = await http.get(Uri.parse("http://$deviceIP:$port/api/restaurants/${widget.restaurantId}/"));
      final testRes = await http.get(Uri.parse("http://$deviceIP:$port/api/testimonials/?search=${widget.restaurantId}"));
  ...
  Widget _sectionTitle(String title) {
    return Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Color(0xFF1A1A1A)));
  }

  Widget _buildHeroImage() {
    final rawUrl = restaurantData?['image'] ?? "";
    final imageUrl = rawUrl.isEmpty ? "https://via.placeholder.com/600" : "http://$deviceIP:$port$rawUrl";
    return SizedBox(
      height: 350,
      width: double.infinity,
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            _headerButton(Icons.arrow_back_ios_new, () => Navigator.pop(context)),
            _headerButton(
  ...
  Widget _headerButton(IconData icon, VoidCallback onTap, {Color color = Colors.black}) {
    return Container(
    ...
    return Column(
      children: testimonials.map((t) => Padding(
        ...
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const CircleAvatar(radius: 20, backgroundColor: Color(0xFFFFEBEE), child: Icon(Icons.person_rounded, color: Colors.redAccent)),
            const SizedBox(width: 15),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(t['creator_name'] ?? "Guest", style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
                  const SizedBox(height: 5),
                  Text(t['text'] ?? "", style: TextStyle(color: Colors.grey[600], height: 1.4, fontSize: 14)),
                ],
              ),
            ),
          ],
  ...
  Widget _buildDateTimePicker() {
    return Column(
      children: [
        _buildPickerTile(Icons.calendar_today_rounded, "Reservation Date", DateFormat('EEEE, d MMMM').format(selectedDate), () async {
          final date = await showDatePicker(context: context, initialDate: selectedDate, firstDate: DateTime.now(), lastDate: DateTime.now().add(const Duration(days: 60)));
          if (date != null) setState(() => selectedDate = date);
        }),
        const SizedBox(height: 12),
        Row(
          children: [
            Expanded(child: _buildPickerTile(Icons.more_time_rounded, "Arrival", startTime.format(context), () async {
              final time = await showTimePicker(context: context, initialTime: startTime);
              if (time != null) setState(() => startTime = time);
            })),
            const SizedBox(width: 12),
            Expanded(child: _buildPickerTile(Icons.history_rounded, "Departure", endTime.format(context), () async {
  ...
  Widget _buildPickerTile(IconData icon, String label, String value, VoidCallback onTap) {
    return InkWell(
      onTap: onTap,
    ...
    return Wrap(
      ...
      children: types.map((s) {
        ...
        return FilterChip(
          label: Text(s['name']),
          selected: isSelected,
          onSelected: (val) => setState(() => selectedSeating = val ? s['name'] : null),
  ...
  Widget _buildSubmitButton() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.redAccent.withOpacity(0.3), blurRadius: 15, offset: Offset(0, 8))],
      ),
      child: ElevatedButton(
        ...
        onPressed: () {},
        child: const Text("Book a Table", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
      ),
    );
  }
}